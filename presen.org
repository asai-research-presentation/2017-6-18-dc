...
** Very similar concept was used for satisficing search, with a different context
...
|                          | Preferred Ops | ✗            |
|--------------------------+---------------+--------------|
| Minimize heuristic eval. | Lazy Eval     | ○           |
|--------------------------+---------------+--------------|
...
Previous methods have the mixed effect of both (e.g. GBFS-LE)
...
** Remove the effect of heuristics from the discussion
...
#+end_span6
#+begin_span6
+ Both modes are *heuristic-agnostic*
  + → based on a knowledge-free (blind) method
...
** Analysing the blind search behavior
...
+ W/O state information (e.g. atoms) --- for continuous domains (e.g. RRT)

[[png:fractals/shape]]

** Fractals: A mathematical tool to analyse shapes
...
#+end_span6
#+begin_span6
Random fractals (we are interested in these)
...
** Fractal dimension: A mathematical tool to analyse fractals
...
A practical diversification method based on a fractal

[[png:fractals/bip]]

** Improves performance as a diversification method (ICAPS17)
...
(Randomized) search algorithm == Generator of a (random) fractal
...
** Fractals offer a mathematical tools to analyse algorithms
...
+ Unified Search Theory^{TM}:
  + Optimal search (A*) is a sequence of satisficing search
  + Satisficing heuristic search is a combination of blind search
  + Fractal theory provides mathematical tools for understanding blind search

* Old materials                                                    :noexport:
